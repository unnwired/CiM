...
DefaultDirName={autopf}\Charts In Motion
...
UninstallDisplayIcon={app}\desktop\flowx.ico
...
begin
  if MachineCodeEdit.Text = '' then
    Exit;
  TempPath := ExpandConstant('{tmp}\flowx-machine-code.txt');
...
begin
  ...
  begin
    if (StoredInstallKey <> '') and (not FlowXLicenseFileExists) then
      EnsureFlowXLicenseFile(StoredMachineCode, StoredInstallKey);
  end;
  if CurStep = ssPostInstall then
  begin
    EnsureFlowXLicenseFile(StoredMachineCode, StoredInstallKey);
    SaveStringToFile(GetFlowXInstallDir + '\version.txt', '{#AppVersion}', False);
  end;
end;
