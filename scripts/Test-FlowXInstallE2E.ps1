...
Write-Host "=== FlowX install E2E (isolated) ==="
...
Write-Host "[5/6] Running FlowXSetup (silent)..."
$logFile = Join-Path $env:TEMP "flowx-e2e-install.log"
...
if ($p.ExitCode -ne 0) {
    ...
    throw "FlowXSetup exit code $($p.ExitCode). See log: $logFile"
}

Write-Host "[6/6] Verifying license + decrypt..."
$licensePath = Join-Path $InstallDir "data\.flowx-license"
...
Write-Host "[7/7] Backend bootstrap smoke (flowx_bootstrap -> server.server)..."
$installEsc = $InstallDir.Replace("'", "''")
$smoke = & $py -s -c @"
import os, sys
sys.path.insert(0, r'$installEsc')
os.environ.pop('FLOWX_LICENSE_SECRET', None)
from server.flowx_bootstrap import _get_app
app = _get_app()
