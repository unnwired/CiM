...
function Test-FlowXShowApplyDialog {
    if ($env:FLOWX_NO_PAUSE -eq "1") { return $false }
    try {
...
function Show-FlowXApplyDialog([string]$Text, [string]$Icon = "Information") {
    if (-not (Test-FlowXShowApplyDialog)) {
        Write-Log "Skipping dialog (FLOWX_NO_PAUSE or non-interactive): $Text"
        return
    }
    Add-Type -AssemblyName System.Windows.Forms
    [System.Windows.Forms.MessageBox]::Show($Text, "FlowX Update", "OK", $Icon) | Out-Null
}
...
try {
    ...
    # Always refresh vendor secret when the update ships it (must match encrypted server/*.enc).
    $vendorRel = "config\.fx-dist.cfg"
    $vendorSrc = Join-Path $sourceDir $vendorRel
    if (-not (Test-Path -LiteralPath $vendorSrc)) {
        $vendorSrc = Join-Path $sourceDir ("payload\" + $vendorRel)
    }
    if (Test-Path -LiteralPath $vendorSrc) {
        $vendorDst = Join-Path $InstallRoot $vendorRel
        $parent = Split-Path -Parent $vendorDst
        if ($parent -and -not (Test-Path -LiteralPath $parent)) {
            New-Item -ItemType Directory -Force -Path $parent | Out-Null
        }
        Copy-Item -LiteralPath $vendorSrc -Destination $vendorDst -Force
        Write-Log "Copied $vendorRel (decrypt key)"
    }
    ...
    $msg = "Update complete ($ver). Start FlowX from start_flowx.bat or the desktop shortcut."
