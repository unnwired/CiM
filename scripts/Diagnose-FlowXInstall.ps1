...
<#
.SYNOPSIS
  Run on the CLIENT PC from the FlowX install folder. Reports common install/startup issues.
#>
[CmdletBinding()]
...
Write-Host ""
Write-Host "========== FlowX install diagnostic =========="
...
$checks = @(
    @{ Path = "start_flowx.bat"; Label = "Launcher" },
    ...
    @{ Path = "data\.flowx-license"; Label = "License file" },
    @{ Path = "config\.fx-dist.cfg"; Label = "Distribution profile" }
)
foreach ($c in $checks) {
...
$legacyProfile = Join-Path $InstallRoot "config\.flowx_vendor_secret"
if (-not (Test-Path -LiteralPath (Join-Path $InstallRoot "config\.fx-dist.cfg")) -and
    (Test-Path -LiteralPath $legacyProfile)) {
    Report "Legacy distribution profile" $true $legacyProfile
}
...
if ([bool]$env:FLOWX_LICENSE_SECRET) {
    Report "FLOWX_LICENSE_SECRET env unset" $false "Env is set; Repair clears it for installed apps."
} else {
    Report "FLOWX_LICENSE_SECRET env unset" $true
}
...
Write-Host ""
if ($fail -eq 0) {
    Write-Host "All checks passed. If UI still fails, run start_flowx.bat and watch the window."
    ...
    Write-Host "  1. Repair-FlowXLicense.ps1 (if license fail)"
