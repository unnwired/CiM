...
.DESCRIPTION
  Run at the start of Build-CiM.ps1 / build_distribution_full.ps1 so missing
...
$requiredSources = @(
    @{ Path = $fx.CiMIss; Label = "Inno Setup script" },
    ...
    @{ Path = (Join-Path $RepoRoot "export_cim.ps1"); Label = "Export script" },
...
if (-not $SkipEncrypt) {
    $hasSecret = $false
    if ($env:CIM_LICENSE_SECRET -and $env:CIM_LICENSE_SECRET.Trim()) {
        $hasSecret = $true
    } else {
...
if ($missing.Count -eq 0) {
    Write-Host "[ok] Charts In Motion build prerequisites verified."
    exit 0
}

Write-Host ""
Write-Host "Charts In Motion build prerequisites FAILED ($($missing.Count) issue(s)):" -ForegroundColor Red
