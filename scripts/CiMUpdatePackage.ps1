...
<#
.SYNOPSIS
  Locate CiM-Update-* packages under install\UPDATE and stage them for apply.
#>
...
function Get-CiMUpdatePackageName([string]$PackageRoot) {
    $leaf = Split-Path -Leaf (Normalize-FlowXPath $PackageRoot)
    if ($leaf -match '\^CiM-Update') { return $leaf }
    ...
    if (Test-Path -LiteralPath $manifest) {
        try {
            ...
            if ($ver) { return "CiM-Update-$ver" }
        } catch { }
    }
    return "CiM-Update"
}

function Find-CiMUpdatePackageRoots {
    param(
...
function Find-CiMUpdatePackageRoot {
    param(
        [Parameter(Mandatory = $true)]
        [string]$InstallRoot
    )
    $packages = Find-CiMUpdatePackageRoots -InstallRoot $InstallRoot
...
function Copy-UpdatePackageToInstall {
    ...
    $folderName = Get-CiMUpdatePackageName -PackageRoot $package
...
function Get-UpdateFolderDiagnosis {
    ...
    $nested = @($names | Where-Object { $_ -match '\^CiM-Update' })
    if ($nested.Count -gt 0) {
        return "UPDATE contains CiM-Update package(s): $($nested -join ', ')"
    }
    ...
    if ($hasPayload -and -not $hasManifest) {
        return "UPDATE has payload\ but no update.manifest.json. Run Install-Client-Update.bat from the CiM-Update ZIP folder inside UPDATE\."
    }
