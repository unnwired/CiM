...
class AppCaches:
    def __init__(self) -> None:
        ...
        self._combined_filter_cache: TTLCache = self._new_filter_cache()
    ...
    def set_mode(self, aggressive: bool) -> None:
        ...
        with self._filter_lock:
            ...
            self._combined_filter_cache = self._new_filter_cache()
    ...
    def invalidate_filters(self) -> None:
        with self._filter_lock:
            ...
            self._combined_filter_cache.clear()
        with self._snapshot_lock:
            self._snapshot_filter_coverage_cache = {}

    def get_combined_filter(self, cache_key: str) -> Optional[frozenset]:
        with self._filter_lock:
            val = self._combined_filter_cache.get(cache_key)
            return val if isinstance(val, frozenset) else None

    def set_combined_filter(self, cache_key: str, symbols: frozenset) -> None:
        with self._filter_lock:
            self._combined_filter_cache[cache_key] = symbols
