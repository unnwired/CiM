...
class ScrapeIndicesLogTest(unittest.TestCase):
    ...
    def test_nifty_healthcare_in_equity_catalog(self):
        scrape = _import_scrape_indices()
        from server.nse_constituents import INDEX_ARCHIVE_CSV, NSE_INDEX_MAP

        symbols = [s for s, _n, c in scrape.INDICES if c == "equity"]
        self.assertIn("NIFTY_HEALTHCARE.NS", symbols)
        self.assertEqual(scrape.NSE_NAME_MAP.get("NIFTY_HEALTHCARE.NS"), "NIFTY HEALTHCARE INDEX")
        self.assertEqual(NSE_INDEX_MAP.get("NIFTY_HEALTHCARE.NS"), "NIFTY HEALTHCARE INDEX")
        self.assertEqual(
            INDEX_ARCHIVE_CSV.get("NIFTY_HEALTHCARE.NS"),
