...
class ApiIntegrationTests(unittest.TestCase):
    ...
    def test_filter_presets_save_writes_file(self):
        import server.server as srv

        with tempfile.TemporaryDirectory() as tmp:
            preset_path = Path(tmp) / "saved_filters.json"
            orig_presets_path = srv._presets_path
            srv._presets_path = lambda: preset_path
            try:
                payload = {
                    "name": "Regression preset",
                    "filters": [{"field": "sector", "op": "eq", "value": "IT"}],
                }
                r = self.client.post("/api/filter-presets", json=payload)
                self.assertEqual(r.status_code, 200, r.text)
                self.assertTrue(preset_path.is_file())
                saved = json.loads(preset_path.read_text(encoding="utf-8"))
                self.assertEqual(len(saved), 1)
                self.assertEqual(saved[0]["name"], "Regression preset")
            finally:
                srv._presets_path = orig_presets_path
