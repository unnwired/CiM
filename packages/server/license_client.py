...
from server.client_user_agent import merge_client_env_body
from server.http_ssl import https_ssl_context
...
def worker_login(
    ...
    merge_client_env_body(body, client_env)
...
def worker_heartbeat(
    ...
    if client_ip:
        body["client_ip"] = client_ip
    merge_client_env_body(body, client_env)
    return _worker_request("POST", "/license/heartbeat", body=body, bearer=access_token)
...
def login_and_create_browser_session(
    base_dir: Path,
    email: str,
    password: str,
    totp_code: Optional[str] = None,
    ...
    session: dict[str, Any] = {"email": resp.get("email") or email}
    _apply_token_response(session, resp)
    session_id = ss.create_session(base_dir, session)
    return session_id, resp


def logout_browser_session(base_dir: Path, session_id: str) -> None:
    from server import session_store as ss

    session = ss.load_session_by_id(base_dir, session_id)
    if session:
        worker_logout(session)
    ss.delete_session_by_id(base_dir, session_id)
...
def license_status_for_client(
    base_dir: Path,
    session: Optional[dict[str, Any]] = None,
    *,
    host_mode: Optional[str] = None,
) -> dict[str, Any]:
    from server.product_config import is_web_host_mode, is_showcase_host, require_online_auth
    from server.web_auth import load_showcase_install_settings

    if host_mode is None:
        host_mode = "web" if is_web_host_mode(base_dir) else "desktop"

    base_status: dict[str, Any] = {
        "host_mode": host_mode,
        "is_showcase_host": is_showcase_host(base_dir) if host_mode == "web" else True,
    }
    if host_mode == "web":
        showcase = load_showcase_install_settings(base_dir)
        role = str(showcase.get("role") or "").strip().lower()
        if role:
            base_status["showcase_role"] = role
        label = str(showcase.get("label") or "").strip()
        if label:
    ...
    if session is None and host_mode == "web":
        return {**base_status, "mode": "none", "valid": False}
