...
const WL_COLS = [
  ...
  { key: 'act', widthKey: 'remove', label: '', width: 34, sortKey: null },
];
...
export default function WatchlistPage({
  ...
  const liveStocksMap = useMemo(() => {
    ...
    Object.keys(out).forEach((sym) => {
      out[sym] = overlayStockRow(out[sym], sym);
    });
    return out;
  }, [stocksMap, overlayStockRow, patchRefreshTick]);
  const liveIndicesMap = useMemo(() => {
    const out = { ...indicesMap };
    Object.keys(out).forEach((sym) => {
      if (out[sym]) out[sym] = overlayIndexRow(out[sym]);
    });
    return out;
  }, [indicesMap, overlayIndexRow, patchRefreshTick]);
  const { getPanelHeights, heightsRef: currentHeightsRef, handleHeightsChange, applyLayoutHeights, heightsRevision } = useSyncedPanelHeights({
    ...
    localStorageKey: chartPrefs?.enabled ? null : 'watchlist.panelHeights',
  });
  ...
  const { startResize: startColResize, resizingKey: colResizingKey, gridTemplateColumns } = useStockListColumnWidths(WL_COLS);
  ...
  const baseDisplayWlItems = useMemo(() => {
    ...
    return sortWatchlistItems(orderedWatchlistItems, wlSortKey, wlSortDir, liveStocksMap, liveIndicesMap);
  }, [orderedWatchlistItems, wlOrderMode, wlSortKey, wlSortDir, liveStocksMap, liveIndicesMap]);
  ...
  useRegisterFocusedSymbol('watchlist', useMemo(
  ...
  const { layoutHydrated, indicatorsHydrated } = useWebChartLayoutMount(CHART_PAGE_IDS.watchlist, {
    setChartLayout, setTimeframe, setTimeframe2, setTimeframe3, setPaneWidth,
  }, (data, fromPrefs, globalIndicator) => {
    const savedItemOrder = data[LIST_ORDER_KEYS.watchlistItemOrder];
    if (savedItemOrder && typeof savedItemOrder === 'object') {
      setWatchlistItemOrder(savedItemOrder);
    }
    hydrateIndicatorPanels(data, fromPrefs, globalIndicator, { applyLayoutHeights, setPanelOrder });
  });

  useWebChartLayoutAutoSave(CHART_PAGE_IDS.watchlist, {
  ...
  }, [paneWidth, activeWatchlist?.name, watchlists.length, gridTemplateColumns]);
    ...
    try {
      ...
      setWatchlistItemOrder(prev => {
        ...
        persistLayoutOrderFields(chartPrefs?.email, {
          [LIST_ORDER_KEYS.watchlistItemOrder]: next,
        });
        return next;
      });
      await onWatchlistsChange();
  ...
  async function handleImportWatchlistFile(e) {
    ...
    try {
      ...
      if (data.watchlist_item_order && typeof data.watchlist_item_order === 'object') {
        ...
        persistLayoutOrderFields(chartPrefs?.email, {
          [LIST_ORDER_KEYS.watchlistItemOrder]: data.watchlist_item_order,
        });
      }
      await onWatchlistsChange();
  ...
  function persistWatchlistItemOrder(nextMap) {
    ...
    persistLayoutOrderFields(chartPrefs?.email, {
      [LIST_ORDER_KEYS.watchlistItemOrder]: nextMap,
    });
  }
  ...
  function handleSaveLayout() {
    ...
    const payload = mergeIndicatorPanelSaveFields({
      ...pageLayoutToApiPayload(CHART_PAGE_IDS.watchlist, snapshot),
      watchlistItemOrder,
    }, currentHeightsRef.current, panelOrder);
    saveWebChartLayoutOrServer(CHART_PAGE_IDS.watchlist, chartPrefs, snapshot, payload);
  }
  ...
  function renderPanel(item, tf, setTf, onLast, cacheKey, hasBorderRight, columnIndex) {
    if (!item) return null;
      ...
      <div {...common}>
        ...
        {item.type === 'stock' ? (
          <ChartContainer
            symbol={item.symbol}
            timeframe={tf}
            pricePanelTitle={activeItemData?.['Market Sector'] || '—'}
            liveToday={tfLive}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%', overflow: 'hidden', fontSize: 12 }}>
      <div className="chart-app-toolbar" style={{ height: 48, borderBottom: '1px solid var(--border)', backgroundColor: 'var(--bg-secondary)', display: 'flex', alignItems: 'center', padding: '0 12px', gap: 8, overflowX: 'auto', fontSize: 12 }}>
        ...
        {activeItemData && (
          <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--text-primary)', fontWeight: 700, fontSize: 14, lineHeight: 1.1, flexShrink: 0 }}>
            {selectedItem?.symbol}
          </span>
        )}
        {activeItemData && (
      ...
      <div ref={wrapperRef} style={{ flex: 1, minHeight: 0, display: 'flex', overflow: 'hidden' }}>
        <div ref={paneRef} style={{ width: paneWidth, minWidth: 300, borderRight: '1px solid var(--border)', flexShrink: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            {WL_COLS.map((col, colIdx) => {
              ...
              return (
                <StockListColumnHeader
                  key={col.key}
                  colKey={col.key}
                  isLast={colIdx === WL_COLS.length - 1}
                  widthKey={wk}
                  resizingKey={colResizingKey}
                  onResizeStart={(e) => startColResize(col, e)}
                  sortable={sortable}
                  onClick={sortable ? () => handleWlSort(col.sortKey) : undefined}
                >
          ...
          {displayWlItems.map((it, wi) => {
            const data = it.type === 'index' ? liveIndicesMap[it.symbol] : liveStocksMap[it.symbol];
            ...
            return (
              ...
              >
                <StockListGridCell colKey="symbol" compact={canDrag}>
                  {canDrag ? (
                    <span
                ...
                <StockListGridCell colKey="act" isLast>
        ...
        <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          {!selectedItem ? (
            ...
            <div style={{ flex: 1, minHeight: 0, display: 'flex', overflow: 'hidden', position: 'relative' }}>
              ...
              {renderPanel(selectedItem, timeframe, setTimeframe, (pct, price) => { setLastChange(pct); setLastPrice(price); }, `${selectedItem.symbol}-p1-${timeframe}`, chartLayout !== 'single', 0)}
              {(chartLayout === '2h' || chartLayout === '3h') && renderPanel(selectedItem, timeframe2, setTimeframe2, () => {}, `${selectedItem.symbol}-p2-${timeframe2}`, chartLayout === '3h', 1)}
              {chartLayout === '3h' && renderPanel(selectedItem, timeframe3, setTimeframe3, () => {}, `${selectedItem.symbol}-p3-${timeframe3}`, false, 2)}
            </div>
