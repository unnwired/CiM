...
export default function PotentialSwingsPage({
  ...
  const { liveActive, liveTick } = usePageLive('potential-swings');
  const { overlayGenericRow, refreshTick: patchRefreshTick } = usePatchOverlay('potential-swings');
  const liveRows = useMemo(
    () => rows.map((r) => overlayGenericRow(r)),
    [rows, overlayGenericRow, patchRefreshTick],
  );
  useRegisterFocusedSymbol('potential-swings', useMemo(
  ...
  const { layoutHydrated, indicatorsHydrated } = useWebChartLayoutMount(CHART_PAGE_IDS.potentialSwings, {
  ...
  useWebChartLayoutAutoSave(CHART_PAGE_IDS.potentialSwings, {
  ...
  useEffect(() => {
    const preset = SCREENER_GROUPS.flatMap((g) => g.items).find((p) => p.key === activePreset);
    if (!preset) return undefined;
    const onRefresh = () => { runPreset(preset); };
  ...
  function handleSaveLayout() {
    ...
    const payload = mergeIndicatorPanelSaveFields(
      pageLayoutToApiPayload(CHART_PAGE_IDS.potentialSwings, snapshot),
      heightsRef.current,
      panelOrder,
    );
    saveWebChartLayoutOrServer(CHART_PAGE_IDS.potentialSwings, chartPrefs, snapshot, payload);
  }

  function renderPanel(sym, tf, setTf, onLastChange, cacheKey, hasBorderRight, columnIndex) {
    if (!sym) return null;
            ...
            >
              <DrawingWorkspaceProvider workspaceId={`potential-swings-${selectedSymbol}`}>
                <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minWidth: 400, position: 'relative' }}>
                  ...
                  {renderPanel(selectedSymbol, timeframe, tf => setTimeframe(tf), pct => setLastCandleChange(pct), `${selectedSymbol}-p1-${timeframe}`, chartLayout !== 'single', 0)}
                  {(chartLayout === '2h' || chartLayout === '3h') && renderPanel(selectedSymbol, timeframe2, tf => setTimeframe2(tf), () => {}, `${selectedSymbol}-p2-${timeframe2}`, chartLayout === '3h', 1)}
                  {chartLayout === '3h' && renderPanel(selectedSymbol, timeframe3, tf => setTimeframe3(tf), () => {}, `${selectedSymbol}-p3-${timeframe3}`, false, 2)}
                </div>
