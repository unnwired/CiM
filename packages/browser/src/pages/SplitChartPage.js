...
import { resolveIndicatorPanelLayout } from '../chartPrefs/chartPageLayout';
...
export default function SplitChartPage({ symbol: initialSymbol, onOpenChart, onOpenFinancials, onActiveSymbolChange }) {
  ...
  const { getPanelHeights, heightsRef, handleHeightsChange, applyLayoutHeights, heightsRevision } = useSyncedPanelHeights({
    columnCount: columnCountForChartLayout(layout),
  });
  ...
  const { liveActive, liveTick } = usePageLive('chart');
  useRegisterFocusedSymbol('chart', useMemo(() => {
    const set = new Set();
  ...
  useEffect(() => {
    axios.get(`${API}/api/layout`).then(r => {
      ...
      const prefs = (chartPrefs?.enabled && chartPrefs?.email)
        ...
        : null;
      applyLayoutHeights(resolveIndicatorPanelLayout(d, null, prefs?.indicatorPanels));
      setIndicatorsHydrated(true);
    }).catch(() => setIndicatorsHydrated(true));
  }, [initialSymbol, applyLayoutHeights, chartPrefs?.enabled, chartPrefs?.email]);
...
function defaultPanelTfForGroup(g) {
  return g === 'D' ? '1D' : PANEL_TF_GROUPS[g][0];
}
...
function PanelHeader({ panel, idx, isActive, showHeader, onTimeframeChange }) {
  ...
  function handleGroupClick(g) {
    setActiveGroup(g);
    onTimeframeChange(defaultPanelTfForGroup(g));
  }
  return (
...
// ── Layout grid ───────────────────────────────────────────────────────────────
function LayoutGrid({ layout, panels, activeIdx, onActivate, onUpdatePanel, onHeightsChange, getPanelHeights, heightsRevision, layoutKey, crosshairTime, onCrosshairMove, liveActive = false, liveTick = 0 }) {
  const columnCount = columnCountForChartLayout(layoutKey);
  function renderPanel(idx) {
    const p = panels[idx];
    if (!p) return null;
    const isActive = idx === activeIdx;
    const showHeader = panels.length > 1 || !!p.symbol;
    const tfLive = liveActive && isIntradayLiveTimeframe(p.timeframe);
    return (
      <div
        key={idx}
      ...
      >
        {p.symbol ? (
          ...
          <ChartContainer
            ...
            onHeightsChange={h => onHeightsChange(h, idx)}
            {...multiColumnHeightProps({
              columnCount,
              columnIndex: idx,
              getPanelHeights,
              heightsRevision,
