...
import { dispatchAppDataRefresh } from '../chartEvents';
...
export default function AdminPanel({
  ...
  const { status, startPolling, refreshStatus } = useAdminJobStatus({
    ...
    onFinished: (next) => {
      ...
      if (!next.error) {
        dispatchAppDataRefresh({ job: next.job });
      }
    },
  });
  ...
  const panelW    = tab === 'sectors' ? 920 : (tab === 'chartPrefs' ? 520 : 480);
    ...
    >
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 18px', borderBottom: '1px solid var(--border)', backgroundColor: 'var(--bg-tertiary)', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ display: 'flex', borderRadius: 5, overflow: 'hidden', border: '1px solid var(--border)' }}>
              ...
              {showChartPrefsTab && (
                <button type="button" onClick={() => setTab('chartPrefs')} style={{
                  padding: '5px 12px', fontSize: 11, border: 'none', cursor: 'pointer',
                  backgroundColor: tab === 'chartPrefs' ? 'var(--accent-blue)' : 'var(--bg-secondary)', color: tab === 'chartPrefs' ? '#fff' : 'var(--text-secondary)',
                }}>Chart preferences</button>
              )}
              {showSectorsTab && (
        ...
        <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16, overflowY: 'auto', flex: 1 }}>
          ...
          {showChartPrefsTab && tab === 'chartPrefs' && (
            <>
              <div style={sectionStyle}>
                <div style={labelStyle}>Browser chart preferences</div>
                <div style={descStyle}>
                  Stored in this browser only for your login. Sign-out does not erase these settings.
                  Other devices or browsers will not sync automatically. Clearing site data removes them.
                </div>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                  <div><strong>EMAs visible:</strong> {chartPrefs?.summary?.emasOn || '—'}</div>
                  <div><strong>Indicator panels:</strong> {chartPrefs?.summary?.panels || '—'}</div>
                  <div><strong>Volume:</strong> {chartPrefs?.summary?.volume || '—'}</div>
                  <div><strong>Pages with saved layout:</strong> {chartPrefs?.summary?.layoutPages || '—'}</div>
                  {chartPrefs?.summary?.savedAt && (
                    <div style={{ marginTop: 6, color: 'var(--text-muted)', fontSize: 11 }}>
                      Last saved: {String(chartPrefs.summary.savedAt).slice(0, 19).replace('T', ' ')}
                    </div>
                  )}
                </div>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 4 }}>
                  <button
                    type="button"
                    onClick={() => chartPrefs?.saveNow?.()}
                    style={{ ...btnBase, backgroundColor: '#238636' }}
                  >
                    Save now
                  </button>
                  <button
                    type="button"
                    onClick={() => chartPrefs?.resetToDefaults?.()}
                    style={{ ...btnBase, backgroundColor: '#6e40c9' }}
                  >
                    Reset to defaults
                  </button>
                </div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center' }}>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      setMapMsg('');
                      try {
                        await axios.post(`${API}/api/admin/refresh-screener-market-sets`);
                        setMapMsg('Auto sector lists refresh started (includes NSE universe expand). Check Jobs tab.');
                        startPolling();
                      } catch (e) {
                        setMapMsg(e.response?.data?.detail || e.message);
                      }
                    ...
                    disabled={isRunning}
                    style={{ ...btnBase, backgroundColor: '#8957e5', height: 34 }}
                  >
                    Refresh auto sector lists
                  </button>
                  <button
                    type="button"
                    onClick={async () => {
                      setMapMsg('');
                      try {
                        await axios.post(`${API}/api/admin/expand-screener-universe`);
                        setMapMsg('Universe expand job started — adds NSE-listed symbols from auto market sets. Check Jobs tab.');
                        startPolling();
                      } catch (e) {
                        setMapMsg(e.response?.data?.detail || e.message);
                      }
