...
const SIDE_OPTIONS = [
  { key: 'positive', label: 'Positive', desc: 'Selected-side bars stay above zero' },
  { key: 'negative', label: 'Negative', desc: 'Selected-side bars stay below zero' },
];
...
function parseAllowCrossZero(value) {
...
export default function MACDHistChainFilterBuilder({ onApply, onCancel, initialValues }) {
  ...
  function chipLabel() {
    ...
    const sideLabel = histogramSide === 'positive' ? 'pos' : 'neg';
    const parts = [
      `MACD Histogram (${timeframe})`,
      modeLabel,
      `main ${barsToCompare || '4'} bars`,
      `stragglers ${allowedStragglers || '0'}`,
      sideLabel,
    ];
    if (allowCrossZero) {
      const crossPrefix = isIncreasing ? 'before' : 'after';
      parts.push(`cross ${crossPrefix} ${crossZeroBars || '3'}`);
      parts.push(`cz stragglers ${crossZeroStragglers || '0'}`);
    }
    return parts.join(' · ');
  }
    ...
    >
      <div style={{ backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: 8, width: 520, boxShadow: '0 16px 48px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 18, maxHeight: 'calc(90vh - 52px)', overflowY: 'auto' }}>
          ...
          <div style={crossZeroSectionStyle}>
