  ...
  try {
    if (isDistributionProfile) {
      ...
      if (migrated !== DIST_EMA_DEFAULTS_VERSION) {
        let normalizedDefault = normalizeEmaSet(DEFAULT_EMAS);
        const rawExisting = window.localStorage.getItem(EMA_PREFS_KEY);
        if (rawExisting) {
          try {
            const prior = normalizeEmaSet(JSON.parse(rawExisting));
            const priorByPeriod = new Map(prior.map((e) => [Number(e.period), e]));
            normalizedDefault = normalizedDefault.map((ema) => {
              const priorEma = priorByPeriod.get(ema.period);
              return priorEma?.color
                ? { ...ema, color: priorEma.color }
                : ema;
            });
          } catch {
            // Keep distribution defaults when prior prefs are unreadable.
          }
        }
