...
async function decryptSupportQrObjectUrl() {
...
/** Synchronous read of session cache (null until first prefetch/load completes). */
export function getCachedSecureSupportQrUrl() {
  return cachedObjectUrl;
}

/** Decrypt once, cache blob URL for the rest of the page session. */
export function prefetchSecureSupportQr() {
  if (cachedObjectUrl) return Promise.resolve(cachedObjectUrl);
  if (!prefetchPromise) {
    prefetchPromise = decryptSupportQrObjectUrl()
      .then((url) => {
        cachedObjectUrl = url;
        return url;
      })
...
/**
 * Return cached blob URL or decrypt on first use.
 * @returns {Promise<string>}
 */
export async function loadSecureSupportQrObjectUrl() {
  return prefetchSecureSupportQr();
}
