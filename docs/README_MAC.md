...
Run CiM as a native desktop app (same Electron window as on Windows).
...
Or use the one-click script with the `--electron` flag:

```bash
bash start_cim_mac.sh --electron
```

> **Important:** The Electron window loads `http://127.0.0.1:8000`, so the backend **must** be running before opening Electron.
...
### Electron window stuck on "Waiting for backend…"

Make sure backend is already running before launching Electron:

```bash
# Terminal 1 — backend
python -m uvicorn server.server:app --host 127.0.0.1 --port 8000

# Terminal 2 — Electron (only after backend is up)
...
# START (every time)
bash start_cim_mac.sh                # browser mode (recommended)
bash start_cim_mac.sh --electron     # Electron desktop mode
