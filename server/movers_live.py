"""
Live NSE quotes for Market Movers.

- Bulk equity-stockIndices (existing index names) updates many symbols per request.
- Rotating quote-equity batches refresh remaining screener symbols without hitting NSE every 5–10s.
...
def warm_cache_on_startup() -> None:
    """One-shot NSE bulk refresh so dashboard 1D % has live quotes soon after server boot."""
    try:
        _run_nse_refresh_once(include_quotes=True)
...
def configure_poll_interval(seconds: int) -> dict[str, Any]:
    ...
    if sec == 0:
        ...
        _ensure_worker()
        threading.Thread(target=_run_nse_refresh_once, name="movers-live-initial", daemon=True).start()
    return get_status()
...
def refresh_after_data_update(*, include_quotes: bool = True) -> None:
    """After OHLCV / universal update: warm NSE cache so movers show today's session."""
    global _all_symbols
    ...
    threading.Thread(target=_go, name="movers-post-data-nse", daemon=True).start()


def _ensure_bulk_cache_fresh() -> None:
    """Refresh NSE bulk indices when cache is empty or older than BULK_REFRESH_MIN_SEC."""
    global _last_bulk_refresh_ts
    with _lock:
    ...
    if cache_len >= 400 and age < BULK_REFRESH_MIN_SEC:
        return
    _run_nse_refresh_once()


def _symbol_needs_live_quote(sym: str) -> bool:
    ...
    if not snap:
        return True
    return _finite_or_none(snap.get("price")) is None


def _fetch_missing_quote_symbols(symbols: list[str], *, max_n: int = ON_DEMAND_QUOTE_MAX) -> int:
    """quote-equity for symbols missing live price (top movers are often outside bulk indices)."""
    seen: set[str] = set()
    missing: list[str] = []
    for sym in symbols:
        s = str(sym or "").strip().upper()
...
def get_symbol_live_snapshot(symbol: str, *, allow_fetch: bool = False) -> Optional[dict[str, Any]]:
    ...
    if snap and _finite_or_none(snap.get("price")) is not None:
        return snap
    if allow_fetch:
        _fetch_missing_quote_symbols([sym], max_n=1)
        snap = live_cache_snapshot().get(sym)
        if snap and _finite_or_none(snap.get("price")) is not None:
            return snap
    return None
...
def merge_live_into_daily_bars(
    ...
    op = _finite_or_none(snap.get("open"))
    hi = _finite_or_none(snap.get("high"))
    lo = _finite_or_none(snap.get("low"))
    if op is None and prev is not None:
        op = prev
    if op is None:
        op = _finite_or_none(bars[-1].get("close")) or px
    if hi is None:
        hi = px
    if lo is None:
        lo = px
    ...
    if last_day == today:
        last["close"] = px
        last["open"] = round(float(last.get("open") or op), 2)
        last["high"] = round(max(float(last.get("high", px)), hi, px), 2)
        last["low"] = round(min(float(last.get("low", px)), lo, px), 2)
        if vol > 0:
