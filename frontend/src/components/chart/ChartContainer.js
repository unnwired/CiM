...
function seriesValueAtTime(data, time) {
  ...
  for (const d of data) {
    const diff = Math.abs(new Date(normalizeChartTime(d.time)) - new Date(norm));
    if (diff < minDiff) {
      minDiff = diff;
      closest = d;
    }
...
function barCloseAtTime(bars, time) {
  ...
  let minDiff = Infinity;
  for (const b of bars) {
    const diff = Math.abs(new Date(normalizeChartTime(b.time)) - new Date(norm));
    if (diff < minDiff) {
      minDiff = diff;
      closest = b;
    }
...
function changePctAtBarIndex(bars, idx, chartData, dayChangePctOverride, timeframe = '1D') {
  ...
  if (idx === n - 1 && (!timeframe || timeframe === '1D')) {
...
fetch('/api/layout')
  .then(r => r.json())
  .then(data => {
    if (data.stochrsi) globalPanelHeights.stochrsi = data.stochrsi;
    if (data.macd)     globalPanelHeights.macd     = data.macd;
  })
  .catch(() => {});
...
function PricePanel({
  width, height, chartData, timeframe = '1D', emas, volumeVisible, syncRef, syncMutedRef, onHoverTime, onLastChange, onCrosshairMove, crosshairTime,
  dayChangePctOverride,
  earningsEvents = [],
...
export default function ChartContainer({
  ...
  const seededHeightsRef = useRef(false);

  useEffect(() => {
    if (!panelHeights) return;
    if (seededHeightsRef.current) return;
    const next = { ...globalPanelHeights };
    if (panelHeights.stochrsi != null) next.stochrsi = panelHeights.stochrsi;
    if (panelHeights.macd != null) next.macd = panelHeights.macd;
    Object.assign(globalPanelHeights, next);
    setManualHeights(next);
    seededHeightsRef.current = true;
  }, [panelHeights?.stochrsi, panelHeights?.macd]);
  ...
  }, [isPreloaded, symbol, timeframe, emaKey, liveToday]);
  ...
  useEffect(() => {
    ...
    cb(Math.round(pct * 100) / 100, last.close);
  }, [isPreloaded, loading, error, chartData]);
  ...
  useEffect(() => {
    const onChartDataUpdated = () => reloadChart();
          ...
          {activePanels.map((key, idx) => {
            ...
            return (
              <React.Fragment key={key}>
                ...
                {key === 'stochrsi' && <StochRSIPanel key={`stochrsi-${dims.width}`} {...commonProps} hoverTime={hoverTime} />}
                {key === 'macd'     && <MACDPanel     key={`macd-${dims.width}`}     {...commonProps} hoverTime={hoverTime} />}
              </React.Fragment>
            );
          })}
        </>
