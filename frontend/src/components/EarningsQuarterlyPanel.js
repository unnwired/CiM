...
export function EarningsQuarterlyPanelContent({ symbol, profileWidthPx, scrollTableToLatest = false }) {
  ...
  const load = useCallback(async ({ fetchIfMissing = false, refresh = false } = {}) => {
    if (!symbol) return;
    ...
    try {
      const r = refresh
        ...
        : await axios.get(`${API}/api/screener-quarters/${encodeURIComponent(symbol)}`, {
            params: { basis, fetch_if_missing: fetchIfMissing },
          });
      setData(r.data?.data ?? null);
      setStatus(r.data?.status ?? null);
    } catch (err) {
      const msg = err.response?.data?.detail || err.message || 'Failed to load quarterly data';
  ...
  useEffect(() => {
    ...
    if (symbol) load({ fetchIfMissing: true });
  }, [symbol, basis, load]);
