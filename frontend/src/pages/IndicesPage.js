...
export default function IndicesPage({ onOpenConstituents, onContextMenuRequest }) {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/api/layout`).then(r => {
      if (r.data.indicesPaneWidth)   setPaneWidth(r.data.indicesPaneWidth);
      if (r.data.indicesChartLayout) setChartLayout(r.data.indicesChartLayout);
      if (r.data.indicesTimeframe) setTimeframe(r.data.indicesTimeframe);
      if (r.data.indicesTimeframe2) setTimeframe2(r.data.indicesTimeframe2);
      if (r.data.indicesTimeframe3) setTimeframe3(normalizeSavedTimeframe3(r.data.indicesTimeframe3));
      if (r.data.indicesEquitySymbolOrder && Array.isArray(r.data.indicesEquitySymbolOrder)) {
        setEquitySymbolOrder(r.data.indicesEquitySymbolOrder);
      }
    }).catch(() => {});
  }, [loadIndices]);
  ...
  const equity = useMemo(() => {
    const raw = indices.filter(i => i.category === 'equity');
    return applySavedOrder(raw, equitySymbolOrder, i => i.symbol);
  }, [indices, equitySymbolOrder]);
  ...
  function handleSaveLayout() {
    const payload = {
      ...heightsRef.current,
      indicesPaneWidth: paneWidth,
      indicesChartLayout: chartLayout,
      indicesTimeframe: timeframe,
      indicesTimeframe2: timeframe2,
      indicesTimeframe3: timeframe3,
  ...
  function persistEquityOrder(symbols) {
    ...
    axios.post(`${API}/api/layout`, { indicesEquitySymbolOrder: symbols }).catch(() => {});
  }
  ...
  function renderPanel(sym, name, tf, setTf, onLastChange, cacheKey, hasBorderRight) {
    if (!sym) return null;
    return (
        ...
        <div style={{ flex:1, overflow:'hidden', display:'flex', minWidth:400 }}>
          {!selected ? (
            ...
            <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minWidth: 400, position: 'relative' }}>
              ...
              {renderPanel(selected.symbol, selected.name, timeframe,  tf => { setTimeframe(tf); setLastChange(null); }, handlePanelLastChange, selected.symbol+'-p1-'+timeframe,  chartLayout!=='single')}
              {(chartLayout==='2h'||chartLayout==='3h') && renderPanel(selected.symbol, selected.name, timeframe2, tf => setTimeframe2(tf), ()=>{}, selected.symbol+'-p2-'+timeframe2, chartLayout==='3h')}
              {chartLayout==='3h' && renderPanel(selected.symbol, selected.name, timeframe3, tf => setTimeframe3(tf), ()=>{}, selected.symbol+'-p3-'+timeframe3, false)}
            </div>
...
function IndexRow({ index, i, selected, onSelect, rowRefs, onContextMenuRequest, draggable, onDragStartRow, onDragOverRow, onDropRow, onDragEndRow, dragDimmed }) {
