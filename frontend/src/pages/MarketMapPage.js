...
export default function MarketMapPage({ onOpenChart, isActive }) {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/api/layout`).then(r => {
      ...
      if (Array.isArray(r.data.marketMapIndexOrder)) setIndexOrder(r.data.marketMapIndexOrder);
  ...
  useEffect(() => {
    ...
    const match = orderedRows.find(r => r.symbol === selected?.symbol);
    if (!match && orderedRows[0]) setSelected(orderedRows[0]);
    else if (match && match.name && selected?.name !== match.name) {
      setSelected(match);
    }
  }, [orderedRows, selected?.symbol]);

  function persistIndexOrder(symbols) {
    setIndexOrder(symbols);
    axios.post(`${API}/api/layout`, { marketMapIndexOrder: symbols }).catch(() => {});
  }

  function handleSaveLayout() {
    const payload = {
      marketMapPaneWidth: paneWidth,
      ...(indexOrder?.length ? { marketMapIndexOrder: indexOrder } : {}),
  ...
  function onDragOver(e, i) {
    ...
    e.preventDefault();
    const gap = insertionGapFromRowHover(e.clientY, e.currentTarget, i, orderedRows.length);
  ...
  function onDrop(e) {
    ...
    const syms = orderedRows.map(r => r.symbol);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%', backgroundColor: 'var(--bg-primary)', overflow: 'hidden' }}>
      ...
      <div ref={wrapperRef} style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
        ...
        >
          ...
          {!loadingSummary && orderedRows.length > 0 && (
            <>
              ...
              {orderedRows.map((row, i) => (
                <React.Fragment key={row.symbol}>
                  <IndexRailRow
                    ...
                    rowRefs={rowRefs}
                    draggable={orderedRows.length > 1}
