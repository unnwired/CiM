...
if (-not $ExportRoot) {
    $ExportRoot = Join-Path $SourceRoot "installer\output\FlowX"
}
...
function Remove-DirectorySafe {
    ...
    try {
        ...
        # Locked .pyd/.dll (FlowX backend running) — try rename-aside then delete later.
...
function Commit-ExportStaging {
    ...
    if (Test-Path -LiteralPath $FinalRoot) {
        ...
        try {
            ...
            Write-Host "[WARN] Could not rename live export (folder may be open in Explorer or FlowX is running)."
            Write-Host "[WARN] Will merge staging into the live folder instead. Close FlowX and retry export to use a clean swap."
        }
    }
...
trap {
    if (-not $exportSucceeded) {
        ...
        if (Test-Path -LiteralPath $ExportRoot) {
            ...
            Write-Host "[WARN] Do not run start_flowx.bat from that folder. Re-run export after closing FlowX/Explorer."
        }
    }
    break
}
...
if (Test-Path -LiteralPath $StagingExportRoot) {
    ...
    try {
        ...
        Write-Host "[WARN] Close FlowX and Explorer windows on the export folder before re-exporting."
    }
}
...
if ($SkipEmbeddedPython) {
    ...
    Write-Host "(runtime\python is not deleted while FlowX is running.)"
...
# start_flowx.bat and distribution installs depend on this helper script.
...
# Prepare desktop runtime inside export so start_flowx.bat can launch a window immediately.
...
# ------------------------------------------------------------
$dataFiles = @("nse_data.db", "nse_dataset.csv", "nse_calendar.json", "market_sectors.json", "market_sector_mapping.json", "symbol_lineage.json")
if (-not $isDistribution) {
...
foreach ($f in @("github_updates.json", "update_manifest_url.json")) {
